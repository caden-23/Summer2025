...
                    Debug.Log($":no_entry_sign: Disabled {script.GetType().Name} on {droppedObj.name}");
                }
            }
...
            Debug.Log($":white_check_mark: {droppedObj.name} dropped. Hover disabled and gravity active.");
        }
